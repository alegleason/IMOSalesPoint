...
>>>>>>> 22408f508e1cb158071ed17531441eca9f3e298e
           if($row["disponible"]==1){
               $aux=$row["idProducto"];
               $num=$row["disponible"];
               $response.="<div class=\"col m2\"><a class=\"waves-effect waves-light btn green darken-1 three-row-btn valign-wrapper\" id=\"$aux\" name=\"$aux\" value=\"$aux\" onclick=\"cambiarDisponibilidadComidas($aux,$num)\">".$row["nombre"]."</a></div>";
           }else{
               $aux=$row["idProducto"];
               $num=$row["disponible"];
